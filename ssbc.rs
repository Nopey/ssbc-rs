///! Version of main.rs with no dependancies, for compiling with rustc 1.14.1
///! compile with `rustc --edition 2018 ssbc.rs`
    ...
    pub fn repl(&mut self) {
        loop {
            ...
            if !std::io::stdin().read_line(&mut command).is_ok(){
                println!("FATAL ERR: Couldn't read operator's command");
                return;
            }
            match command.chars().next().unwrap_or(' ') {
                ...
                'q' => return,
                _ => {
                    println!("FATAL ERR: Unknown command");
                    return
                }
            }
        }
